...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const modalStatBox = document.querySelector('.modal-stat-box'); // Select the box to hide it
    const closeModal = document.querySelector('.close-modal');
    // Select both pillar cards and specifically marked team cards
    const clickableCards = document.querySelectorAll('.pillar-card, .clickable-card');

    function openModal(card) {
        const topic = card.getAttribute('data-topic');
        const role = card.getAttribute('data-role'); // For team members
        const summary = card.getAttribute('data-summary');
        const stat = card.getAttribute('data-stat');
        const statDesc = card.getAttribute('data-stat-desc');

        // Logic for Title (Name or Topic)
        modalTitle.textContent = topic;

        // If it's a team member, we might want to show the role prominently. 
        // We can append it to the title or put it in the summary.
        // Let's modify the modal structure dynamically or just prepend to summary?
        // Current requirement: "In the pop-up, display their Full Name 'Mandila', Debate Role..., and Topic Summary"

        if (role) {
            // If a role exists, we can display it. 
            // We'll reset the summary content first to avoid duplication if we reopen.
            modalSummary.innerHTML = `<strong>Role: ${role}</strong><br><br>${summary}`;
        } else {
        ...
        if (stat && stat.trim() !== "") {
            ...
            if (modalStatBox) modalStatBox.style.display = 'flex';
        } else {
            if (modalStatBox) modalStatBox.style.display = 'none';
        }
